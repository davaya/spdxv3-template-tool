...
def scan_template_file(file: str, category: str, fname: str) -> dict:
    """
        ...
        for d2 in t2['dirs']:
            ...
            for f3 in t3['files']:
                if f3.name.startswith('_'):
                    ...
                    templ[d1.name][d2.name][fname] = scan_template_file(doc, d2.name, fname)
    return dict(templ)
...
if __name__ == '__main__':
    ...
    template = load_template_from_list_dirs(BASE)['Core']
    print(f'\nConverting to information model ...')
    ...
    schema = {'info': {'package': 'http://spdx.com/v3/core'},
              'types': []}
